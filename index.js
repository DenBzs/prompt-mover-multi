...
function applyGroup(pn, gi) {
    ...
    try {
        ...
        for (const t of g.toggles) {
            ...
            // On/Off = fixed, skip entirely — only 연동(null) follows the group
...
function wireGroupCards(area) {
    ...
    // group on/off — 고정(null) 토글은 건드리지 않고, On/Off 토글만 적용
    area.querySelectorAll('.ptm-onoff').forEach(btn => btn.addEventListener('click', () => {
        ...
        // do NOT reset overrides — 고정(null) toggles are immune
        applyGroup(pn, gi);
        saveGroups(pn, gs);
        renderTGGroups();
    }));
    // individual override: cycle null(연동) → true(On 고정) → false(Off 고정) → null(연동)
