...
// Self-contained, no dependency on chat-toggle-groups
...
// ═══════════════════════════════════════════════════
// A. Toggle Group Data Layer
// extension_settings[TG_KEY].presets[presetName] = Group[]
// Group = { name, isOn, toggles: [{target, behavior}] }
// behavior: 'direct' | 'invert'
// ═══════════════════════════════════════════════════
...
function buildTGSection() {
    const presets = getPresetOptions();
    return `
        <div class="pmm-block">
            <label class="pmm-label">프리셋 선택</label>
            <select id="tg-preset" class="pmm-sel">${presets}</select>
        </div>
        <div id="tg-groups-area"><div class="pmm-ph">프리셋을 선택하세요</div></div>
        <button class="pmm-sm pmm-sm-full" id="tg-add-group" style="display:none">+ 그룹 추가</button>
    `;
}

function renderTGGroups() {
    const area = document.getElementById('tg-groups-area');
    if (!area) return;
    if (!tgActivePreset) {
        area.innerHTML = '<div class="pmm-ph">프리셋을 선택하세요</div>';
        return;
    }
    const groups = getGroupsForPreset(tgActivePreset);
    if (!groups.length) {
        area.innerHTML = '<div class="pmm-ph">그룹이 없습니다. 아래 [+ 그룹 추가]를 클릭하세요.</div>';
        return;
    }
    area.innerHTML = groups.map((g, gi) => buildGroupCard(g, gi)).join('');
    ...
    area.querySelectorAll('.tg-toggle-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            applyGroup(tgActivePreset, gi);
            saveGroups(tgActivePreset, groups);
            renderTGGroups();
        });
    });
    // Rename
    ...
    // Add toggles (multi-select modal)
...
function buildGroupCard(g, gi) {
    const preset     = tgActivePreset ? openai_settings[openai_setting_names[tgActivePreset]] : null;
    ...
    const toggleRows = g.toggles.map((t, ti) => {
        ...
        return `
            <div class="tg-toggle-row">
                ...
                <select class="tg-behavior-sel pmm-sel-sm" data-gi="${gi}" data-ti="${ti}">
                    <option value="direct" ${t.behavior === 'direct' ? 'selected' : ''}>직접</option>
        ...
        <div class="tg-group-card">
            <div class="tg-group-header">
                ...
                <div class="tg-group-actions">
                    ...
                    <button class="tg-delete-group-btn pmm-icon-btn pmm-icon-danger" data-gi="${gi}">그룹 삭제</button>
                </div>
            </div>
                ...
                ${allPrompts.map(p => {
                    ...
                    return `
                        <label class="tg-modal-item ${exists ? 'tg-modal-item-exists' : ''}">
                            <input type="checkbox" class="pmm-chk" data-id="${p.identifier}"
    ...
    el.innerHTML = `
        <div class="inline-drawer">
            ...
            <div class="inline-drawer-content" id="pmm-content">
                ...
                <div class="pmm-tabs">
                    <button class="pmm-tab pmm-tab-active" data-tab="mover">프롬프트 이동</button>
                    <button class="pmm-tab" data-tab="tg">토글 그룹 관리</button>
                </div>

                <!-- ── TAB: Mover ── -->
                <div id="pmm-tab-mover" class="pmm-tab-pane">
                    <div class="pmm-block">
                    ...
                    <div class="pmm-block pmm-group-block">
                        ...
                        <div id="pmm-group-name-row" class="pmm-group-name-row pmm-hidden">
                ...
                <!-- ── TAB: Toggle Groups ── -->
                <div id="pmm-tab-tg" class="pmm-tab-pane pmm-hidden">
                    ${buildTGSection()}
                </div>
...
function wireEvents() {
    // Tabs
    document.querySelectorAll('.pmm-tab').forEach(tab => {
        tab.addEventListener('click', () => {
            document.querySelectorAll('.pmm-tab').forEach(t => t.classList.remove('pmm-tab-active'));
            document.querySelectorAll('.pmm-tab-pane').forEach(p => p.classList.add('pmm-hidden'));
            tab.classList.add('pmm-tab-active');
            document.getElementById(`pmm-tab-${tab.dataset.tab}`)?.classList.remove('pmm-hidden');
        });
    });
    ...
    document.getElementById('pmm-none')?.addEventListener('click', () => {
        document.querySelectorAll('.pmm-chk').forEach(cb => { cb.checked = false; cb.closest('.pmm-item').classList.remove('pmm-checked'); });
    ...
    // Toggle Groups tab
    document.getElementById('tg-preset')?.addEventListener('change', e => {
        tgActivePreset = e.target.value;
        const addBtn   = document.getElementById('tg-add-group');
        if (addBtn) addBtn.style.display = tgActivePreset ? '' : 'none';
        renderTGGroups();
    });
