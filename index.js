...
async function showAddToggleModal(gi) {
    ...
    const html = `
        <div style="display:flex;gap:6px;margin-bottom:8px">
            <input type="text" id="ptm-msearch" placeholder="검색..."
                style="flex:1;padding:6px 8px;border-radius:5px;border:1px solid #555;background:#222;color:#eee;box-sizing:border-box">
            <button id="ptm-add-range" class="ptm-sm" style="flex-shrink:0">연속</button>
        </div>
        <div id="ptm-mlist" style="max-height:50vh;overflow-y:auto">
            ${prompts.map((p, i) => {
                ...
                return `<label style="display:flex;align-items:center;gap:8px;padding:7px 4px;cursor:${ex?'default':'pointer'};opacity:${ex?'0.45':'1'}">
                    <input type="checkbox" class="ptm-add-cb" data-id="${p.identifier}" data-i="${i}" ${ex?'disabled checked':''}
    ...
    const observer = new MutationObserver(() => {
        ...
        // 연속 버튼
        const rangeBtn = document.getElementById('ptm-add-range');
        if (rangeBtn && !rangeBtn._ptmWired) {
            rangeBtn._ptmWired = true;
            rangeBtn.addEventListener('click', () => {
                if (selectedIndices.size < 2) { toastr.warning('시작과 끝 항목 2개를 선택하세요'); return; }
                const s = [...selectedIndices].sort((a,b)=>a-b), mn = s[0], mx = s[s.length-1];
                document.querySelectorAll('.ptm-add-cb:not(:disabled)').forEach(cb => {
                    const i = +cb.dataset.i;
                    if (i >= mn && i <= mx) { cb.checked = true; selectedIndices.add(i); selectedIds.add(cb.dataset.id); }
                });
            });
        }
        // Checkboxes
        document.querySelectorAll('.ptm-add-cb:not(:disabled)').forEach(cb => {
            ...
            cb.addEventListener('change', () => {
                const i = +cb.dataset.i;
                if (cb.checked) { selectedIds.add(cb.dataset.id); selectedIndices.add(i); }
                else { selectedIds.delete(cb.dataset.id); selectedIndices.delete(i); }
            });
        });
    });
    observer.observe(document.body, { childList: true, subtree: true });
